(function($) {
	...
	function productsInfiniteResult() {
		...
		var currentPage = useAjax
			? parseInt(mohawkInfinite.current_page, 10) || 1
			: 1;
	
		var maxPages = useAjax
			? parseInt(mohawkInfinite.max_pages, 10) || 1
			: 1;
		...
		var spinner = target.find('.loading-container');
		...
		function fetchPage(page) {
			...
			return $.ajax({
				...
				var products = temp.find('.row-products').children();
	
				if (!products.length) {
					return { success: false };
				}
	
				var respMax = maxPages;
	
				temp.find('a.page-numbers, span.page-numbers').each(function () {
					var num = parseInt($(this).text(), 10);
					if (!isNaN(num)) respMax = Math.max(respMax, num);
				});
		...
		function prefetch(page) {
			if (prefetched[page] || page > maxPages) return;
	
			fetchPage(page).done(function (res) {
				var data = useAjax ? res.data : (res.success ? res.data : null);
				if (data && data.html) {
		...
		function append(data) {
			...
			if (data.max_pages) {
				maxPages = parseInt(data.max_pages, 10);
			}
		}
		...
		function loadNextPage() {
			...
			isLoading = true;
			spinner.removeClass('d-none hide');
			...
			if (prefetched[nextPage]) {
				append(prefetched[nextPage]);
				prefetched[nextPage] = null;
				done();
				return;
			}
		...
		// OPTIONAL fallback scroll (lightweight, not polling)
		...
		setTimeout(() => {
			if (loaderEl.length) loadNextPage();
		}, 300);
	}
