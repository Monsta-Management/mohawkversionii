	...
	function productSummaryColorSizesOptions(){
		...
		if (productSummary.length){
			productSummary.each(function() {
				...
				setDefaultVariant(sizeEl, colorsEl_alt);
				setColorOption(sizeEl, option_box);
				setSizeOption(sizeEl, size_box);
			});
		}
		...
		function setDefaultVariant(sizeEl, colorsEl) {
			...
			if (sizeEl.length){
				...
				sizeEl.find('option').each(function() {
					...
					if ( opt_val ){
						...
						if (is_medal && opt_val.toUpperCase().includes('G')) {
							the_default = opt_val;
						}
					}
				});
			...
			productSummary.each(function() {
				...
				if (colorsEl.length){
					...
					colorsEl.find('.option-item').each(function() {
						...
						if (is_medal && color_val.toUpperCase().includes('G')) {
