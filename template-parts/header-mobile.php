	...
	<div class="menu-main-menu-container">
		...
		<ul id="menu-main-menu-1" class="menu">
			<?php
				...
				usort( $sub_cats, function( $a, $b ) use ( $order_map ) {
				...
				foreach ( $sub_cats as $sub_category ) {
					if ( in_array( $sub_category->slug, [ 'ungrouped', 'uncategorized' ], true ) ) {
						continue;
					}
					...
					<div class="menu-item menu-item-type-custom menu-item-object-custom collapsible">
						<a class="view-category" href="<?php echo esc_url( $term_link ); ?>" style="line-height:2!important;position:absolute;left:0;z-index:9;">
						...
						<a class="category-name" href="<?php echo esc_url( $term_link ); ?>">
					...
					<?php
					// Get sub-sub categories (keep alphabetical for performance + clarity).
					$subsub_cats = get_terms( [
						'taxonomy'   => $taxonomy,
						'parent'     => $sub_category->term_id,
						'hide_empty' => false,
					] );

					if ( ! is_wp_error( $subsub_cats ) && ! empty( $subsub_cats ) ) {
						...
						foreach ( $subsub_cats as $subsub_category ) {
							...
							echo '<a href="' . esc_url( $subsub_link ) . '">';
							echo esc_html( $subsub_category->name );
							echo '</a>';
							echo '</div>';
						}
