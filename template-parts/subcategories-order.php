	...
	<?php
		foreach ( $product_cat as $parent_product_cat ) {
			if ( $parent_product_cat->term_id != $ungrouped && $parent_product_cat->name != 'Uncategorized' ) {
				$has_category = ( $category instanceof WP_Term );

				$checked_cat = (
					$has_category &&
					$parent_product_cat->term_id == $category->term_id
				);
		
				$checked_parent = (
					$has_category &&
					$parent_product_cat->term_id == $category->parent
				);
		
				$checked_product = (
					is_product() &&
					!empty( $current_product_cat_ids ) &&
					in_array( $parent_product_cat->term_id, $current_product_cat_ids )
				);
